...
class VariantStats:
    ...
    def __init__(self, vcf_iterator): # is it an inheritance from vcf_iterator? 
        self.vcf = vcf_iterator
        self.stats = None
    ...
    def collect_variants(self):
        ...
        for line in self.vcf:
            ...
            svtype = sv_info[-1].split('=')[1] #SVtype is the last item
            try:
                svlen = int(sv_info[-2].split('=')[1]) #SVLEN is the second last item
            except ValueError:
                svlen = np.NaN
            try:
                position = line_split[0] + ':' + line_split[1] + '-' + sv_info[2].split('=')[1] # END is the third item of sv_infor
                alt_allele_count = 0
                allele_count = 0
            except IndexError:
                position = 'NA'
            ...
            for sample in line_split[9:]:
                ...
                gtPassOrNot=sample.split(':')[4] # If it equals to 'LQ', then the genotyping quality is too low. Need to set it as missing.
                if gtPassOrNot =='LQ':
                    continue
            ...
            variants_stats = [svtype, position, svlen, allele_frequency, allele_count]
            try:
